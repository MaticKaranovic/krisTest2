...
import { textOutputFile } from "./files/text-file";
import { stringOutputFile } from "./files/string-file";
...
Pulsar.export(async (sdk: Supernova, context: PulsarContext): Promise<Array<AnyOutputFile>> => {
  ...
  if (context.brandId) {
    ...
    const brand = brands.find(
      (brand) => brand.id === context.brandId || brand.idInVersion === context.brandId,
    );
    if (!brand) {
  ...
  return [
    ...
    ...(Object.values(TokenType)
      .map((type) => {
        switch (type) {
          case "Typography":
            return textOutputFile(type, tokens, tokenGroups);
          case "String":
            return stringOutputFile(type, tokens, tokenGroups);
          case "Border":
          case "BorderRadius":
          case "Color":
          case "Gradient":
          case "Shadow":
          case "Space":
            return styleOutputFile(type, tokens, tokenGroups);
          default:
            return null;
        }
        // return type === "Typography"
        //   ? textOutputFile(type, tokens, tokenGroups)
